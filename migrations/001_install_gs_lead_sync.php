...
function gs_lead_sync_install()
{
    ...
    if (!$CI->db->table_exists('gs_lead_sync_sheets')) {
        ...
        $CI->dbforge->create_table('gs_lead_sync_sheets', true);
    }

    if (!$CI->db->table_exists('gs_lead_sync_imported')) {
        $CI->dbforge->add_field([
            'id'              => ['type' => 'INT', 'constraint' => 11, 'unsigned' => true, 'auto_increment' => true],
    ...
    if (!$CI->db->table_exists('gs_lead_sync_logs')) {
        ...
        $CI->dbforge->add_key('id', true);
        $CI->dbforge->create_table('gs_lead_sync_logs', true);
    }
}
...
function gs_lead_sync_uninstall()
{
    $CI = &get_instance();
    $CI->load->dbforge();
    $CI->dbforge->drop_table('gs_lead_sync_logs', true);
    $CI->dbforge->drop_table('gs_lead_sync_imported', true);
    $CI->dbforge->drop_table('gs_lead_sync_sheets', true);
}
